...
def _enforce_no_repeat_ngram(prev_output_tokens, log_probs, no_repeat_ngram_size):
    if no_repeat_ngram_size <= 0:
        return log_probs

    B, L = prev_output_tokens.size()
    if L < no_repeat_ngram_size - 1:
        return log_probs

    for i in range(B):
        gen_tokens = prev_output_tokens[i].tolist()
        prefix = tuple(gen_tokens[-(no_repeat_ngram_size - 1):])

        for j in range(len(gen_tokens) - (no_repeat_ngram_size - 1)):
            ngram = tuple(gen_tokens[j: j + (no_repeat_ngram_size - 1)])
            if ngram == prefix:
                forbidden_token = gen_tokens[j + (no_repeat_ngram_size - 1)]
                log_probs[i, forbidden_token] = -float('inf')
...
def pack_wrapper(module, att_feats, att_masks):
    if att_feats is None:
        return None

    if att_masks is not None:
        lengths = att_masks.data.long().sum(1)
        packed, inv_ix = sort_pack_padded_sequence(att_feats, lengths)
        out_packed = module(packed[0])
        return pad_unsort_packed_sequence(PackedSequence(out_packed, packed[1]), inv_ix)
    else:
...
class AttModel(CaptionModel):
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, memory1, seq, p_att_feats, att_masks, memory2=None,
                           fc_feats2=None, att_feats2=None, att_masks2=None, state=None, output_logsoftmax=1):
        xt = self.embed(it)
        output, state = self.core(xt, fc_feats, att_feats, memory1, seq, p_att_feats, state, att_masks2, memory2,
                                  fc_feats2, att_feats2, att_masks)
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}, targets=None, fc_feats2=None, att_feats2=None):
        ...
        no_repeat_ngram_size = opt.get('no_repeat_ngram_size', 0)

        assert sample_n == 1 or sample_n == beam_size // group_size, 'when beam search, sample_n == 1 or beam search'
        batch_size = fc_feats.size(0)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, memory1, seq_mask, seq_mask1, memory2, fc_feats2, att_feats2, att_masks = self._prepare_feature(
            fc_feats, att_feats, att_masks, targets, fc_feats2, att_feats2)
        ...
        logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, memory1, seq_mask1, pp_att_feats,
                                                  p_att_masks, memory2, fc_feats2, att_feats2, att_masks, state)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, memory1, seq_mask1, att_masks, memory2, fc_feats2, att_feats2 = utils.repeat_tensors(
            beam_size,
            [p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, memory1, seq_mask1, att_masks, memory2, fc_feats2, att_feats2]
        )

        self.done_beams = self.beam_search(state, logprobs, p_fc_feats, p_att_feats, memory1, seq_mask1, pp_att_feats,
                                           p_att_masks, memory2, fc_feats2, att_feats2, att_masks, opt=opt)
    ...
    def _sample(self, fc_feats, att_feats, context, targets, fc_feats2, att_feats2, att_masks=None):
        ...
        no_repeat_ngram_size = opt.get('no_repeat_ngram_size', 0)
        if hasattr(self.args, 'no_repeat_ngram_size'):
        ...
        for t in range(self.max_seq_length + 1):
            ...
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state,
                                                      memory1=None, seq_mask=None, output_logsoftmax=output_logsoftmax)
            ...
            if no_repeat_ngram_size > 0 and t > 0:
                logprobs = _enforce_no_repeat_ngram(seq[:, :t], logprobs, no_repeat_ngram_size)
